local log_path = "/tmp/safari-gauntlet-receptionist.log"
local screenshot_path = "/tmp/safari-gauntlet-receptionist.png"
...
cbid = callbacks:add("frame", function()
	...
	elseif phase == "talk" then
		...
		elseif phase_frame == 72 then
			...
			state_line("VERIFIED_RECEPTIONIST")
			apply_keys(0)
			callbacks:remove(cbid)
			return
		end
...
log("Safari Gauntlet receptionist verifier loaded")
state_line("initial")
