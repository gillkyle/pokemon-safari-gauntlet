local log_path = "/tmp/safari-gauntlet-tutor.log"
local screenshot_path = "/tmp/safari-gauntlet-tutor.png"
...
local function state_line(prefix)
	log(string.format(
		"%s map=%d/%d xy=%d,%d pmap=%d,%d dir=%02x status=%02x script=%02x/%02x party=%d",
		prefix,
		read8(W.map_group),
		read8(W.map_number),
		read8(W.x),
		read8(W.y),
		read8(W.player_map_x),
		read8(W.player_map_y),
...
cbid = callbacks:add("keysRead", function()
	...
	elseif phase == "talk" then
		...
		elseif elapsed < 90 then
			keys = 0
		elseif elapsed < 125 then
			keys = A
		elseif elapsed < 180 then
			keys = 0
		elseif elapsed < 215 then
			keys = A
		elseif elapsed < 270 then
			keys = 0
		elseif elapsed < 305 then
			keys = A
		elseif elapsed > 420 then
			emu:screenshot(screenshot_path)
			log("screenshot=" .. screenshot_path)
			state_line("VERIFIED_TUTOR")
			apply_keys(0)
			callbacks:remove(cbid)
			return
		end
...
log("Safari Gauntlet tutor verifier loaded")
state_line("initial")
