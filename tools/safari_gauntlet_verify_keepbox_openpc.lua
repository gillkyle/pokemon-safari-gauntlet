local log_path = "/tmp/safari-gauntlet-keepbox-openpc.log"
local screenshot_path = "/tmp/safari-gauntlet-keepbox-openpc.png"
...
cbid = callbacks:add("frame", function()
	...
	elseif phase == "talk" then
		...
		elseif phase_frame < 36 then
			keys = A
		elseif phase_frame < 56 then
			keys = A
		elseif phase_frame < 76 then
			keys = A
		elseif phase_frame == 140 then
			emu:screenshot(screenshot_path)
			log("screenshot=" .. screenshot_path)
			state_line("VERIFIED_KEEPBOX_OPENPC")
			apply_keys(0)
			callbacks:remove(cbid)
			return
		end
...
log("Safari Gauntlet keepbox open-PC verifier loaded")
state_line("initial")
